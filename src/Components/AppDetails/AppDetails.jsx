...
import { addToStoredDB, installedIds } from '../Utility/addToDB'; 
...
const AppDetails = () => {
    ...
    useEffect(() => {
        const handleStorageChange = () => {
            const stored = installedIds();
            if (stored.includes(parseInt(id))) {
                setInstall('Installed');
                setIsDisable(true);
            } else {
                setInstall(`Install ${size} MB`);
                setIsDisable(false);
            }
        };
            ...
            <div className='flex mx-3.5 gap-x-25'>
                ...
                <div className='flex-2'>
                    ...
                    <p className='text-gray-400'>
                        Developed By:
                        <span className='bg-[linear-gradient(125.07deg,_#632ee3,_#9f62f2)] bg-clip-text text-transparent ml-2'>
                            {companyName}
                        </span>
                    </p>
                    ...
                    <button
                        className={`btn btn-success text-white ${isDisable ? 'opacity-70 cursor-not-allowed' : ''}`}
